...
def cd(url, backlinksFlag = False):
    ...
    file_1 = open('url_cdate.json','a')
    one_element['url'] = url
    one_element['created_date'] = lowest
    file_1.write(json.dumps(one_element)+'\n')
    file_1.close
    return r
    


q2_data=open("mem_grt0.json",'r')
for each_line in q2_data.readlines():
    json_line=json.loads(each_line)
    url=json_line['url']
    cd(url)
